...
`useState` accepts an initial value and returns a two-element tuple: the current state value and a setter function. The generic type parameter `T` is inferred automatically by TypeScript from the initial value you provide.
